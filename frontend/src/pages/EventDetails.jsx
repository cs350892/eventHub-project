...
import { ArrowLeft, Calendar, MapPin, Users, IndianRupee, XCircle, Clock, Heart, Share2, User } from 'lucide-react';
...
function EventDetails() {
  ...
  const isEventPast = (dateString, timeString) => {
    const eventDateTime = new Date(`${new Date(dateString).toISOString().split('T')[0]}T${timeString}`);
  ...
  const handleRegister = async () => {
    try {
      ...
      const data = await res.json();
      if (data._id) {
        setEvent({ ...event, attendeeCount: event.attendeeCount + 1, attendees: [...(event.attendees || []), { user: { name: 'You', email: 'you@example.com' } }] });
        alert('Registered successfully!');
      } else {
        alert(data.error || 'Error registering for event');
      }
    } catch (err) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div className="flex items-center text-gray-600">
                  ...
                  <div>
                    ...
                    <p className="text-sm">{event.attendeeCount} / {event.maxAttendees}</p>
                  </div>
                </div>
                <div className="flex items-center text-gray-600">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 sticky top-8">
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm font-bold text-gray-900">{spotsRemaining} / {event.maxAttendees}</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className={`h-2 rounded-full transition-all duration-300 ${
                    spotsRemaining === 0 ? 'bg-red-500' : spotsRemaining <= 5 ? 'bg-yellow-500' : 'bg-green-500'
                  }`}
                  style={{ width: `${Math.max((event.attendeeCount / event.maxAttendees) * 100, 5)}%` }}
                ></div>
              </div>
              {spotsRemaining === 0 && (
                <p className="text-sm text-red-600 mt-2 font-medium">This event is full</p>
              )}
            </div>
            {!isPastEvent && (
            ...
            {event.attendees && event.attendees.length > 0 && (
              <div className="mt-8 pt-6 border-t">
                <h3 className="text-sm font-semibold text-gray-900 mb-3">Recent Registrations</h3>
                <div className="space-y-2">
                  {event.attendees.slice(0, 5).map((attendee, index) => (
                    <div key={index} className="flex items-center text-sm">
                      ...
                      <span className="text-gray-600">{attendee.user.name}</span>
                    </div>
                  ))}
                  {event.attendees.length > 5 && (
                    <p className="text-xs text-gray-500 mt-2">+{event.attendees.length - 5} more attendees</p>
                  )}
                </div>
              </div>
            )}
